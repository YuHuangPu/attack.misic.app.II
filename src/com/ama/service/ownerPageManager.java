	...
	public String execute(){
		...
		try {
			...
			sql = "SELECT A.SUM, A.SELL_DATE FROM ("
					+"SELECT SUM(PRICE) AS SUM, DATE_FORMAT(SELL_DATE, '%y-%m-%d') AS SELL_DATE FROM GOODS_DETAIL GROUP BY SELL_DATE ORDER BY DATE_FORMAT(SELL_DATE, '%Y-%m-%d') DESC  LIMIT 0, 10"
