...
//	}
	...
	private String execute(HttpServletRequest request, HttpServletResponse response, ora.json.JSONObject reqJson, Connection conn) throws SQLException{
		...
		String delSql = "DELETE FROM `FACTORY` WHERE `FACTORY`.`ID` = ?";
		...
		if(reqJson.length() != 1){
			...
			pstmt.setString(2, reqJson.getString("Contact"));
			pstmt.setString(3, reqJson.getString("Address"));
			pstmt.setString(4, reqJson.getString("Mobile"));
			pstmt.setString(5, reqJson.getString("Remark"));
			pstmt.setString(6, userID);
			pstmt.setTimestamp(7, UpdateDate);
			pstmt.setString(8, reqJson.getString("Id"));
			pstmt.execute();
		}else{
